...
module.exports = {
            ...
            if(Array.isArray(data.unit_id) && Array.isArray(data.co_val)){
                for(let i=0; i<data.unit_id.length; i++){
                    var select = 'id',
                        ...
                        whr = `type_id = ${data.type_id} AND act_id = ${data.act_id} AND emi_type_id = ${data.emi_type_id} AND unit_id = ${data.unit_id[i]} AND repo_period = ${data.repo_period}`,
                        order = null;
                    ...
                    var table_name = 'md_cal_emi_val',
                        ...
                        '(type_id, act_id, emi_type_id, repo_period, unit_id, co_val, created_by, created_dt)',
                        values = `('${data.type_id}', '${data.act_id}', '${data.emi_type_id}', ${data.repo_period}, '${data.unit_id[i]}', '${data.co_val[i]}', '${user}', '${datetime}')`,
                ...
                var select = 'id',
                    table_name = 'md_cal_emi_val',
                    whr = `type_id = ${data.type_id} AND act_id = ${data.act_id} AND emi_type_id = ${data.emi_type_id} AND unit_id = ${data.unit_id} AND repo_period = ${data.repo_period}`,
                    order = null;
                var chk_dt = await db_Select(select, table_name, whr, order)
                ...
                var table_name = 'md_cal_emi_val',
                    ...
                    '(type_id, act_id, emi_type_id, repo_period, unit_id, co_val, created_by, created_dt)',
                    values = `('${data.type_id}', '${data.act_id}', '${data.emi_type_id}', ${data.repo_period}, '${data.unit_id}', '${data.co_val}', '${user}', '${datetime}')`,
                    whr = chk_dt.suc > 0 && chk_dt.msg.length > 0 ? `id = ${chk_dt.msg[0].id}` : null,
                    flag = chk_dt.suc > 0 && chk_dt.msg.length > 0 ? 1 : 0;
                res_dt = await db_Insert(table_name, fields, values, whr, flag)
                resolve(res_dt);
    ...
    getCalUnitList: (type_id = 0, act_id = 0, emi_type_id=0, year=0) => {
        return new Promise(async (resolve, reject) => {
            var select = 'DISTINCT a.unit_id, b.unit_name, a.co_val',
                table_name = 'md_cal_emi_val a, md_unit b',
                whr = `a.unit_id=b.id ${type_id > 0 ? `AND a.type_id = ${type_id}` : ''} ${act_id > 0 ? `AND a.act_id = ${act_id}` : ''} ${emi_type_id > 0 ? `AND a.emi_type_id = ${emi_type_id}` : ''} ${year > 0 ? `AND a.repo_period = ${year}` : ''}`,
                order = null;
            var res_dt = await db_Select(select, table_name, whr, order)
            resolve(res_dt)
        })
