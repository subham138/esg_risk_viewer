...
module.exports = {
    ...
    getCalAct: (id = 0, type_id = 0, flag='E') => {
        return new Promise(async (resolve, reject) => {
            var select = 'a.id, a.type_id, a.act_name, a.map_act_name, b.type_name',
    ...
    saveCalAct: (data, user) => {
        return new Promise(async (resolve, reject) => {
            ...
            var table_name = 'md_cal_act',
                fields = data.id > 0 ? `type_id = '${data.type_id}', act_name = "${data.act_name}", map_act_name = "${data.map_act_name}", modified_by = '${user}', modified_dt = '${datetime}'` : 
            '(flag, type_id, act_name, map_act_name, created_by, created_dt)',
                values = `('${data.flag}', '${data.type_id}', "${data.act_name}", "${data.map_act_name}", '${user}', '${datetime}')`,
            whr = data.id > 0 ? `id = ${data.id}` : null,
            flag = data.id > 0 ? 1 : 0;
            var res_dt = await db_Insert(table_name, fields, values, whr, flag)
            resolve(res_dt)
    ...
    getCalEmiType: (id = 0, type_id = 0, act_id = 0, flag = 'E', view_flag = null) => {
        return new Promise(async (resolve, reject) => {
            ...
            table_name = 'md_cal_emi_type a, md_cal_type b, md_cal_act c',
                whr = `a.type_id=b.id AND a.act_id=c.id AND a.flag = '${flag}' ${id > 0 ? `AND a.id = ${id}` : ''} ${type_id > 0 ? `AND a.type_id = ${type_id}` : ''} ${act_id > 0 ? `AND a.act_id = ${act_id}` : ''} ${view_flag ? `AND a.view_flag = '${view_flag}'` : ''}`,
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if(cal_sec_dt.suc > 0 && cal_sec_dt.msg.length > 0){
                ...
                for(let dt of cal_sec_dt.msg){
                    ...
                    var qstDtlsAndLogic = await db_Select(q_select, 'md_cal_form_builder a, md_cal_form_build_logic b', q_whr, 'ORDER BY a.id asc')
    ...
    getGhgCalList: (proj_id, client_id, period) => {
        return new Promise(async (resolve, reject) => {
            var calSel = `a.id, a.client_id, a.scope, a.project_id, a.quest_id, a.sl_no, a.sec_id, a.act_id, a.emi_type_id, a.repo_period, a.repo_month, a.repo_mode_label, a.emi_type_unit_id, a.cal_val, a.emi_fact_val, a.co_val, c.act_name, d.emi_name, b.sec_id cal_sec_id, e.sec_name cal_sec_name, b.sequence, b.parent_id, b.sub_parent_id, (SELECT quest_ans FROM td_ghg_quest f WHERE f.client_id = a.client_id AND f.scope = a.scope AND f.project_id = a.project_id AND f.pro_sl_no = a.sl_no AND f.proj_year = a.repo_period AND f.quest_type = 'I' AND f.quest_seq LIKE '%.%.1' LIMIT 1 ) quest_ans`,
                calWhr = `a.quest_id=b.id AND a.act_id=c.id AND a.emi_type_id=d.id AND b.sec_id=e.id AND a.project_id = ${proj_id} AND a.client_id = ${client_id} AND a.repo_period=${period} AND a.emi_type_id not in(2425, 2426, 2427, 741, 2431, 2432, 2433, 744)`;
