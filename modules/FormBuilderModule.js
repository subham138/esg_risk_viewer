...
module.exports = {
    get_form_builder_list : (scope_id = 0, sec_id = 0) => {
        return new Promise(async (resolve, reject) => {
            ...
            var resDt = await db_Select('a.*, b.sec_name, b.type_id', 'md_cal_form_builder a, md_cal_sec_type b', `a.sec_id=b.id AND a.scope_id=b.scope_id ${scope_id > 0 ? `AND a.scope_id=${scope_id}` : ''} ${sec_id > 0 ? `AND a.sec_id = ${sec_id}` : ''}`, 'ORDER BY a.id asc')
            if(resDt.suc > 0){
                ...
                res_dt = {suc: 1, msg: {q_header, q_scope, q_type_id, q_sec_name, q_data: questFilterData}}
                resolve(res_dt)
            }else{
                resolve(resDt)
            }
