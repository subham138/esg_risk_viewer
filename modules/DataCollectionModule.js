...
module.exports = {
    ...
    getDynamicData: (id=null, sec_id = null, ind_id = null, topic_id = null) => {
        return new Promise(async (resolve, reject) => {
            ...
            whr = `a.sec_id=b.id AND a.ind_id=c.id AND a.top_id=d.id AND a.ind_id = d.ind_id AND d.topic_id=e.id ${id > 0 ? `a.id = ${id}` : ''} ${sec_id > 0 ? `AND a.sec_id = ${sec_id}` : ''} ${ind_id > 0 ? `AND a.ind_id = ${ind_id}` : ''} ${ind_id > 0 ? `AND a.top_id = ${topic_id}` : ''}`,
            order = 'ORDER BY a.id';
            var res_dt = await db_Select(select, table_name, whr, order)
            resolve(res_dt)
        })
