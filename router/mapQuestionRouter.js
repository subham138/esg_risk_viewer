...
mapQuestionRouter.post('/get_calc_ques_list_ajax', async (req, res) => {
    var dt = req.body
    // console.log(dt,'data');
    var data = await db_Select('*', 'md_cal_form_builder', `scope_id='${dt.scope}' AND sec_id='${dt.sec_id}' AND lang_flag = '${dt.lang_flag}' AND is_parent = 'N' AND is_sub_parent='Y'`, 'ORDER BY id asc')
    res.send(data)
})
