        ...
        if (Array.isArray(data[`code_${dt}`])) {
          for (let i = 0; i < data[`code_${dt}`].length; i++) {
            if(data[`info_title_${dt}`][i] != ''){
              ...
              var table_name = `td_sus_dis_top_met`,
                fields = `info_title = "${data[`info_title_${dt}`][i]}", info_desc = '${data[`info_desc_${dt}`][i] != '' && data[`info_desc_${dt}`][i] ? data[`info_desc_${dt}`][i].split("'").join("\\'") : ""}', modified_by= '${user}', modified_dt = '${datetime}'`,
                values = null,
                whr = chk_whr,
                flag = 1;
              res_dt = await db_Insert(table_name, fields, values, whr, flag);
          ...
          if(data[`info_title_${dt}`] != ''){
            ...
            // console.log(chk_dt);
            var table_name = `td_sus_dis_top_met`,
              fields = `info_title = "${data[`info_title_${dt}`]}", info_desc = '${data[`info_desc_${dt}`] != '' && data[`info_desc_${dt}`] ? data[`info_desc_${dt}`].split("'").join("\\'") : ''}', modified_by= '${user}', modified_dt = '${datetime}'`,
              values = null,
              whr = chk_whr,
              flag = 1;
            res_dt = await db_Insert(table_name, fields, values, whr, flag);
...
DataCollectionRouter.get('/get_sus_disc_word_info_ajax', async (req, res) => {
  var data = req.query
  var res_dt = await getWordInfo(0, data.top_id, data.bus_id, data.word ? data.word : '', data.bus_id ? data.bus_id : 0)
  res.send(res_dt)
})
