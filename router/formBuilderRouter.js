...
FBRouter.post('/cal_sec_type_edit', async (req, res) => {
    ...
    var res_dt = await db_Insert('md_cal_sec_type', data.id > 0 ? `scope_id = ${data.scope_id}, sec_name = '${data.sec_name}', modified_by = '${user}', modified_dt = '${datetime}'` : '(scope_id, sec_name, created_by, created_dt)', `(${data.scope_id}, '${data.sec_name}', '${user}', '${datetime}')`, data.id > 0 ? `id=${data.id}` : null, data.id > 0 ? 1 : 0)
    if(res_dt.suc > 0){
        req.session.message = {
            type: "success",
